...
main.py
-------
Standalone NSE/BSE F&O futures alert bot. Like the MCX commodity bot,
contracts are resolved dynamically (never hardcoded) and re-resolved
daily at market open so expired monthly contracts are replaced
automatically.
...
from fno_futures_feed import feed
...
def resolve_and_sync_symbols():
    symbols = feed.resolve_active_contracts(config.UNDERLYINGS)
    for sym in symbols:
...
def check_all_symbols():
    ...
    log.info(f"Scanning {len(symbols)} resolved F&O contracts...")
...
def bootstrap():
    ...
    if config.DRY_RUN:
        mock_symbols = [f"{u['name']}MOCK" for u in config.UNDERLYINGS]
        for sym in mock_symbols:
            storage._seed_symbols([sym])
            feed.instrument_map[sym] = {"token": "0", "base": sym, "exch_seg": "NFO"}
        log.info(f"[FNO] DRY_RUN=true, using {len(mock_symbols)} mock symbols instead of real contract resolution")
    else:
        resolve_and_sync_symbols()
...
def health():
    return jsonify({
        ...
        "total_underlyings": len(config.UNDERLYINGS),
...
def resolve_contracts_endpoint():
    ...
    return jsonify({"status": "resolved", "contract_count": len(symbols), "contracts": symbols})
